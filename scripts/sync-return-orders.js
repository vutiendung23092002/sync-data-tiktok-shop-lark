...
try {
  for (const shopConfig of env.shops) {
    ...
    const range = getSyncRange(env, undefined, { maxTo: "today" });
    const returnOrders = await tiktok.searchReturns({ shopCipher: context.shopCipher, createTimeGe: range.from, createTimeLt: range.to });
    ...
    const result = await service.sync({ returnOrders, shop: { shopId: context.shopId, shopName: context.shopName }, range });
    logger.info({ ...result, range, shopId: context.shopId }, "Return Orders sync completed");
  }
} finally {
